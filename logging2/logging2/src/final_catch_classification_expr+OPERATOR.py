...
result_table = "result_catch_expr_operator"
"""
port=3307
user="sangeetal"
password="sangeetal"
...
str_logged = "select   exp, logged from "+ table_catch_feature +" where exp!='' and \
 logged= 1"
...
#======== Not Logged===========#
str_not_logged = "select    exp, logged from "+table_catch_feature+" where \
exp!='' and logged= 0"
     ...
     for i in range(1,1001):
         for j in range(1, 1001):
             for k in range(1, 1001):
                 ...
                 rf_f1 = cross_validation.cross_val_score(rf, x_data_tfidf.toarray(), target_arr, cv=cv, scoring='f1')
                 n_str = "insert into rf_temp_result_catch values(\'"+project+"\',"+(str)(i)+","+(str)(j)+","+(str)(k)+","+(str)(rf_acc.mean())+","+(str)(rf_f1.mean())+")"
                 insert_cursor.execute(n_str)
                 db1.commit() 
"""            
"""
