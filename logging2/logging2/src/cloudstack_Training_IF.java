...
public class cloudstack_Training_IF {
	...
	{				    
		cloudstack_Training_IF demo = new cloudstack_Training_IF();
...
public void ast_prser(String file_name)
	{
		try
		{
			...
			rawContent = cloudstack_Training_IF.readFileToString(file_name);
    ...
    block.accept(new ASTVisitor() 
    {
        ...
        public boolean visit(IfStatement myif) 
        {
        	...
        	//String parent_content= myif.getParent().toString();
             ...
             if_train_con = method_content.substring(0, if_pos);
             //if_train_con = if_train_con.replace("\n", " "); //make it comment for parsing
           	 if_train_con = if_train_con.replace("\"", "\\\"");
           	 if_train_con = if_train_con.replace("\'", "\\\'"); 
          	 if_train_con = if_train_con.replace("\\\\", " ");
          	 //if_train_con = if_train_con.replaceAll("\n", " "); //make it comment for parsing
             if_train_con =if_train_con.replaceAll("\\s+", " ");
