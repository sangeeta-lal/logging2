...
temp_result_table = "temp_result_catch_training2_textual_features_selection"
"""
port=3307
user="sangeetal"
password="sangeetal"
...
for  random_seed_val_cross_validation in rand_array:
    ...
    total_features= total_data.shape[1]
    ...
    #========================Random Forest===================================#
        ...
        rf_score = cross_validation.cross_val_score(rf,trunc_total_data.toarray()  , 
                                                np.asarray(target), cv=cv)
        rf_acc = cross_validation.cross_val_score(rf,trunc_total_data.toarray(), 
             np.asarray(target), cv=cv, scoring='accuracy')
        rf_precision = cross_validation.cross_val_score(rf,trunc_total_data.toarray(), 
                  np.asarray(target), cv=cv,scoring='precision')
        rf_recall = cross_validation.cross_val_score(rf,trunc_total_data.toarray(), 
                np.asarray(target), cv=cv,scoring='recall')
        rf_f1= cross_validation.cross_val_score(rf,trunc_total_data.toarray(), 
                np.asarray(target), cv=cv,scoring='f1')
        rf_roc = cross_validation.cross_val_score(rf,trunc_total_data.toarray(),
