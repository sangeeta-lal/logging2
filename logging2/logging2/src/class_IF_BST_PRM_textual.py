...
final_result_table = "final_result_table_if_training2_textual_features"
"""
port=3307
user="sangeetal"
password="sangeetal"
...
for d in logged_data:
   ...
   # logged_if_n_features.append(temp)     
    logged_if_t_features.append(text_features)
    target.append(1)                  

#====Inserting for negative class======#
for i  in range(len(logged_if_t_features)):
    target.append(0)
    
print "len=", len(logged_if_t_features)    , "targte= ", len(target)
...
for  random_seed_val_cross_validation in rand_array:
    ...
    indices = list()
    indices = np.random.permutation(len(non_logged_data))[:len(logged_if_t_features)]

    print "len not logged tuples=", len(non_logged_data), " indices len=", len(indices),  "  logged if t features=", len(logged_if_t_features)
    ...
    print "len of features=", len(total_if_t_features)
    
    vectorizer = TfidfVectorizer(min_df=1)   
    total_data =vectorizer.fit_transform(np.asarray(total_if_t_features))
